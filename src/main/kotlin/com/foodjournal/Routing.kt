...
fun Application.configureRouting() {
    ...
    val incomesService by inject<IncomesService>()
    val goalsService by inject<GoalsService>()
    ...
    routing {
        ...
        post("/accounts") {
            ...
            if ((user.login.length > 20) or (user.login.length <= 3)) {
                ...
                call.respondText("Imvalid Email", status = HttpStatusCode.ExpectationFailed)
            } else {
                try {
        ...
        authenticate("auth-session") {
            ...
            put("/accounts") {
                ...
                if (session == null) {
                    ...
                    val user = ExposedUser(login, pass, email);
    ...
    routing {
        ...
        post("/login") {
            ...
            if (com.foodjournal.authenticate(user.login, user.pass)) {
                val email = userService.read(user.login)!!.email
                    call.sessions.set(UserSession(user.login, user.pass, email))
